...
#define p33_cs_h(x)        do{local_irq_disable();\
							if(x&BIT(15)){UNI_P33->SPI_CON |= BIT(0)|BIT(8);}\
							else{UNI_P33->SPI_CON  &= ~BIT(8);UNI_P33->SPI_CON  |= BIT(0);}}while(0)

#define p33_cs_l           do{UNI_P33->SPI_CON  &= ~(BIT(0)|BIT(8));local_irq_enable();}while(0)
