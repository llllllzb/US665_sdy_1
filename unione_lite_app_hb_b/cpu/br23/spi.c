...
static UNI_SPI_TypeDef *const spi_regs[SPI_MAX_HW_NUM] = {
    UNI_SPI0,
    UNI_SPI1,
    UNI_SPI2,
};
...
static void spi0_iomc_config(spi_dev spi)
{
    ...
    if (port == 'A') {
        UNI_IOMAP->CON0 &= ~(BIT(16) | BIT(17));
    } else if (port == 'B') {
        UNI_IOMAP->CON0 &= ~(BIT(16) | BIT(17));
        UNI_IOMAP->CON0 |= (BIT(16));
    } else if (port == 'C') {
        UNI_IOMAP->CON0 &= ~(BIT(16) | BIT(17));
        UNI_IOMAP->CON0 |= (BIT(17));
    } else {
        UNI_IOMAP->CON0 |= (BIT(16) | BIT(17));
    }
}
...
void spi_set_bit_mode(spi_dev spi, int mode)
{
    ...
    port -= 'A';
    if (id == SPI2 && port + 'A' == 'B') {
        UNI_USB_IO->CON0 |= BIT(11);  //IO MODE
...
void spi_close(spi_dev spi)
{
    ...
    if (id == SPI2 && port + 'A' == 'B') {
        ...
        UNI_USB_IO->CON0 &= ~BIT(9);  //DP 1.2V digital input dis
        UNI_USB_IO->CON0 &= ~BIT(10);  //DM 1.2V digital input dis
    } else {
