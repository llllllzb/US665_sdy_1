...
//========================= 0.模块开关相关寄存器
#define LED_PWM_ENABLE 					(UNI_PLED->CON0 |= BIT(0))
#define LED_PWM_DISABLE 				(UNI_PLED->CON0 &= ~BIT(0))
#define IS_PWM_LED_ON    				(UNI_PLED->CON0 & BIT(0))

#define RESET_PWM_CON0 					(UNI_PLED->CON0 = 0)
#define RESET_PWM_CON1 					(UNI_PLED->CON1 = 0)
#define RESET_PWM_CON2 					(UNI_PLED->CON2 = 0)
#define RESET_PWM_CON3 					(UNI_PLED->CON3 = 0)
...
#define LED_PWM0_CLK_SEL(x)			PWM_LED_SFR(UNI_PLED->CON0, 2, 2, x)
//SOURCE -- DIV  --> PWM0
//[1:0] 00:div1  01:div4  10:div16  11:div64
//[2]:  0:x1     1:x2
//[3]:  0:x1     1:x256
//DIV = [1:0] * [2] * [3]
#define LED_PWM0_CLK_DIV(x)			PWM_LED_SFR(UNI_PLED->CON0, 4, 4, x)
//PWM0 -- DIV --> PWM1
//PWM_CON3[3:0] -- PWM_PRD_DIVL[7:0]  //12bit
#define LED_PWM1_CLK_DIV(x)	   	    	{do {UNI_PLED->PRD_DIVL = ((x-1) & 0xFF); UNI_PLED->CON3 &= ~(0xF); UNI_PLED->CON3 |= (((x-1) >> 8) & 0xF);} while(0);}

//========================= 2.亮度设置相关寄存器
//最高亮度级数设置
//PWM_BRI_PRDH[1:0] -- PWM_BRI_PRDL[7:0]  //10bit
#define LED_BRI_DUTY_CYCLE(x)			{do {UNI_PLED->BRI_PRDL = (x & 0xFF); UNI_PLED->BRI_PRDH = ((x >> 8) & 0x3);} while(0);}
//高电平亮灯亮度设置
#define LED_BRI_DUTY1_SET(x)			{do {UNI_PLED->BRI_DUTY0L = (x & 0xFF); UNI_PLED->BRI_DUTY0H = ((x >> 8) & 0x3);} while(0);}
//低电平亮灯亮度设置
#define LED_BRI_DUTY0_SET(x)			{do {UNI_PLED->BRI_DUTY1L = (x & 0xFF); UNI_PLED->BRI_DUTY1H = ((x >> 8) & 0x3);} while(0);}
//同步LED0 <-- LED1亮度
#define LED_BRI_DUTY0_SYNC1() 			{do {UNI_PLED->BRI_DUTY0L = UNI_PLED->BRI_DUTY1L; UNI_PLED->BRI_DUTY0H = UNI_PLED->BRI_DUTY1H;} while(0);}
//同步LED1 <-- LED0亮度
#define LED_BRI_DUTY1_SYNC0() 			{do {UNI_PLED->BRI_DUTY1L = UNI_PLED->BRI_DUTY0L; UNI_PLED->BRI_DUTY1H = UNI_PLED->BRI_DUTY0H;} while(0);}
...
//PWM1 duty_cycle 选择
#define LED_PWM1_DUTY_FIX_SET 			(UNI_PLED->CON1 &= ~BIT(3))  //固定PWM1周期为0xFF
#define LED_PWM1_DUTY_VARY_SET(x) 		{do {UNI_PLED->CON1 |= BIT(3); UNI_PLED->DUTY3 = x;} while(0);}

#define LED_PWM1_DUTY0_SET(x)			(UNI_PLED->DUTY0 = x)
#define LED_PWM1_DUTY1_SET(x)			(UNI_PLED->DUTY1 = x)
#define LED_PWM1_DUTY2_SET(x)			(UNI_PLED->DUTY2 = x)
#define LED_PWM1_DUTY3_SET(x)			(UNI_PLED->DUTY3 = x) 	//可以设置为PWM1_DUTY_CYCLE
#define LED_PWM1_PRD_SEL_DIS			(UNI_PLED->CON1 &= ~BIT(3))

#define LED_PWM1_PRD_SEL_EN				(UNI_PLED->CON1 |= BIT(3))
#define LED_PWM1_DUTY0_EN				(UNI_PLED->CON1 |= BIT(4))
#define LED_PWM1_DUTY1_EN				(UNI_PLED->CON1 |= BIT(5))
#define LED_PWM1_DUTY2_EN				(UNI_PLED->CON1 |= BIT(6))
#define LED_PWM1_DUTY3_EN				(UNI_PLED->CON1 |= BIT(7))

#define LED_PWM1_ALL_DUTY_DIS			(UNI_PLED->CON1 &= ~(0xF << 4))
#define LED_PWM1_DUTY0_DIS				(UNI_PLED->CON1 &= ~BIT(4))
#define LED_PWM1_DUTY1_DIS				(UNI_PLED->CON1 &= ~BIT(5))
#define LED_PWM1_DUTY2_DIS				(UNI_PLED->CON1 &= ~BIT(6))
#define LED_PWM1_DUTY3_DIS				(UNI_PLED->CON1 &= ~BIT(7))

//========================= 5.输出取反相关寄存器
//以下几个需要设置为固定值
#define LED_PWM0_INV_DISABLE 			(UNI_PLED->CON1 &= ~BIT(0))
#define LED_PWM1_INV_DISABLE 			(UNI_PLED->CON1 &= ~BIT(1)) //周期从灭灯开始
#define LED_PWM1_INV_ENABLE 			(UNI_PLED->CON1 |= BIT(1))  //周期从亮灯开始
#define LED_PWM_OUT_LOGIC_SET 			(UNI_PLED->CON3 |= BIT(4))
//以下几个可以灵活设置
#define LED_PWM_OUTPUT_INV_ENABLE 		(UNI_PLED->CON1 |= BIT(2))
#define LED_PWM_OUTPUT_INV_DISABLE 		(UNI_PLED->CON1 &= ~BIT(2))

//========================= 6.与周期变色相关寄存器
#define LED_PWM_SHIFT_DUTY_SET(x) 		PWM_LED_SFR(UNI_PLED->CON2, 4, 4, x)

//========================= 7.与驱动强度相关寄存器
#define LED_PWM_IO_MAX_DRIVE(x) 		PWM_LED_SFR(UNI_PLED->CON2, 0, 2, x)

//========================= 8.与中断相关寄存器
#define LED_PWM_INT_EN 					(UNI_PLED->CON3 |= BIT(5))
#define LED_PWM_INT_DIS 				(UNI_PLED->CON3 &= ~BIT(5))
#define LED_PWM_CLR_PENDING 			(UNI_PLED->CON3 |= BIT(6))

//========================= 9.与呼吸模式相关寄存器
#define LED_PWM_BREATHE_ENABLE 			(UNI_PLED->CON0 |= BIT(1))
#define LED_PWM_BREATHE_DISABLE 		(UNI_PLED->CON0 &= ~BIT(1))
//LED呼吸灭灯延时设置, 16bit
...
void log_pwm_led_info()
{
    ...
    led_debug("PWM_CON0 	= 0x%x", UNI_PLED->CON0);
    led_debug("PWM_CON1 	= 0x%x", UNI_PLED->CON1);
    led_debug("PWM_CON2 	= 0x%x", UNI_PLED->CON2);
    led_debug("PWM_CON3 	= 0x%x", UNI_PLED->CON3);
    led_debug("PRD_DIVL     = 0x%x", UNI_PLED->PRD_DIVL);

    led_debug("BRI_PRDL      = 0x%x", UNI_PLED->BRI_PRDL);
    led_debug("BRI_PRDH      = 0x%x", UNI_PLED->BRI_PRDH);

    led_debug("BRI_DUTY0L    = 0x%x", UNI_PLED->BRI_DUTY0L);
    led_debug("BRI_DUTY0H    = 0x%x", UNI_PLED->BRI_DUTY0H);

    led_debug("BRI_DUTY1L    = 0x%x", UNI_PLED->BRI_DUTY1L);
    led_debug("BRI_DUTY1H    = 0x%x", UNI_PLED->BRI_DUTY1H);

    led_debug("PWM1_DUTY0    = 0x%x", UNI_PLED->DUTY0);
    led_debug("PWM1_DUTY1    = 0x%x", UNI_PLED->DUTY1);
    led_debug("PWM1_DUTY2    = 0x%x", UNI_PLED->DUTY2);
    led_debug("PWM1_DUTY3    = 0x%x", UNI_PLED->DUTY3);
}
