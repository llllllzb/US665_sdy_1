...
PWM_TIMER_REG *get_pwm_timer_reg(pwm_timer_num_type index)
{
    ...
    switch (index) {
    case pwm_timer0:
        reg = (PWM_TIMER_REG *)(&(UNI_MCPWM->TMR0_CON));
        break;
    case pwm_timer1:
        reg = (PWM_TIMER_REG *)(&(UNI_MCPWM->TMR1_CON));
        break;
    case pwm_timer2:
        reg = (PWM_TIMER_REG *)(&(UNI_MCPWM->TMR2_CON));
        break;
    case pwm_timer3:
        reg = (PWM_TIMER_REG *)(&(UNI_MCPWM->TMR3_CON));
        break;
    case pwm_timer4:
        reg = (PWM_TIMER_REG *)(&(UNI_MCPWM->TMR4_CON));
        break;
    case pwm_timer5:
        reg = (PWM_TIMER_REG *)(&(UNI_MCPWM->TMR5_CON));
        break;
    default:
        break;
    }
...
PWM_CH_REG *get_pwm_ch_reg(pwm_ch_num_type index)
{
    ...
    switch (index) {
    case pwm_timer0:
        reg = (PWM_CH_REG *)(&(UNI_MCPWM->CH0_CON0));
        break;
    case pwm_timer1:
        reg = (PWM_CH_REG *)(&(UNI_MCPWM->CH1_CON0));
        break;
    case pwm_timer2:
        reg = (PWM_CH_REG *)(&(UNI_MCPWM->CH2_CON0));
        break;
    case pwm_timer3:
        reg = (PWM_CH_REG *)(&(UNI_MCPWM->CH3_CON0));
        break;
    case pwm_timer4:
        reg = (PWM_CH_REG *)(&(UNI_MCPWM->CH4_CON0));
        break;
    case pwm_timer5:
        reg = (PWM_CH_REG *)(&(UNI_MCPWM->CH5_CON0));
        break;
    default:
        break;
    }
...
void mcpwm_set_frequency(pwm_timer_num_type ch, pwm_aligned_mode_type align, u32 frequency)
{
    ...
    mcpwm_div_clk = clk / _pow(2, i); //12000000
...
void mctimer_ch_open_or_close(pwm_timer_num_type timer_ch, u8 enable)
{
    ...
    if (enable) {
        UNI_MCPWM->MCPWM_CON0 |= BIT(timer_ch + 8); //TnEN
    } else {
        UNI_MCPWM->MCPWM_CON0 &= (~BIT(timer_ch + 8)); //TnDIS
    }
}
...
void mcpwm_ch_open_or_close(pwm_ch_num_type pwm_ch, u8 enable)
{
    ...
    if (enable) {
        UNI_MCPWM->MCPWM_CON0 |= BIT(pwm_ch); //PWMnEN
    } else {
        UNI_MCPWM->MCPWM_CON0 &= (~BIT(pwm_ch)); //PWMnDIS
    }
}
...
void log_pwm_info(pwm_ch_num_type pwm_ch, pwm_timer_num_type timer_ch)
{
    ...
    mcpwm_debug("MCPWM_CON0 = 0x%x", UNI_MCPWM->MCPWM_CON0);
...
void mcpwm_init(struct pwm_platform_data *arg)
{
    ...
    if (arg->h_pin == pwm_hw_h_pin[arg->pwm_ch_num]) {    //硬件引脚
        ...
        gpio_set_die(arg->h_pin,1);
        gpio_set_pull_up(arg->h_pin,0);
        gpio_set_pull_down(arg->h_pin,0);
        gpio_set_direction(arg->h_pin,0);
    } else {
...
_CH_L_SET:
    ...
    if (arg->l_pin == pwm_hw_l_pin[arg->pwm_ch_num]) {    //硬件引脚
        ...
        gpio_set_die(arg->l_pin,1);
        gpio_set_pull_up(arg->l_pin,0);
        gpio_set_pull_down(arg->l_pin,0);
        gpio_set_direction(arg->l_pin,0);
    } else {
...
void io_ext_interrupt_init(u8 port, trigger_mode_type trigger_mode, IO_ISR_FUNC cbfun)
{
    ...
    if (trigger_mode) {
        ...
        UNI_MCPWM->FPIN_CON = 0;
    } else {
        gpio_set_pull_up(port, 0);
        gpio_set_pull_down(port, 1);
        UNI_MCPWM->FPIN_CON = BIT(16);
    }
    if (port == IO_PORTA_14) {
        UNI_IOMAP->CON3 &= ~BIT(12);             //使用硬件引脚
    } else {
        UNI_IOMAP->CON3 |= BIT(12);              //使用input_channel 2
        UNI_IOMAP->CON2 &= ~(0b111111 << 16);
        UNI_IOMAP->CON2 |= (port << 16);
    }
    io_isr_cbfun = cbfun;
    request_irq(IRQ_CHX_PWM_IDX, 3, io_interrupt, 0);   //注册中断函数
    UNI_MCPWM->CH0_CON1 = BIT(14) | BIT(11) | BIT(4);
    UNI_MCPWM->MCPWM_CON0 |= BIT(0);
}
...
/**
 * @param UNI_TIMERx : UNI_TIMER0/1/2/3/4/5
 ...
 */
int timer_pwm_init(UNI_TIMER_TypeDef *UNI_TIMERx, u32 fre, u32 duty, u32 port, int output_ch)
{
    u32 hw_port;
    switch ((u32)UNI_TIMERx) {
    case (u32)UNI_TIMER0 :
        hw_port = IO_PORTA_05;
        break;
    case (u32)UNI_TIMER1 :
    ...
    //初始化timer
    UNI_TIMERx->CON = 0;
    UNI_TIMERx->CON |= (0b10 << 2);						//选择晶振时钟源：24MHz
    UNI_TIMERx->CON |= (0b0001 << 4);					//时钟源4分频
    UNI_TIMERx->PRD = OSC_Hz / (4 * fre);				//设置周期
    UNI_TIMERx->PWM = (UNI_TIMERx->PRD * duty) / fre;	//0~10000对应0~100g
    UNI_TIMERx->CNT = 0;									//清计数值
    UNI_TIMERx->CON |= (0b01 << 0);						//计数模式
 ...
 */
void set_timer_pwm_duty(UNI_TIMER_TypeDef *UNI_TIMERx, u32 duty)
{
    UNI_TIMERx->PWM = (UNI_TIMERx->PRD * duty) / 10000;	//0~10000对应0~100%
}

/*  */
/* void timer_pwm_test(void) */
/* { */
/*     timer_pwm_init(UNI_TIMER2, 10000, 2000, IO_PORTB_00, CH2_T2_PWM_OUT); */
/*     timer_pwm_init(UNI_TIMER3, 10000, 5000, IO_PORTB_05, 0); */
/*     timer_pwm_init(UNI_TIMER5, 10000, 8000, IO_PORTB_07, 0); */
/* } */
