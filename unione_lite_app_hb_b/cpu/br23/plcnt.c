...
#define PLCNT_CLOCK_SEL(x)				SFR(UNI_PCNT->CON, 2, 2, x)
#define INPUT_CHANNLE2_SRC_SEL(x)		SFR(UNI_IOMAP->CON2, 16, 6, x)
...
static void scan_capkey(void *priv)
{
    ...
    if (bCapState == 0) {
        bCapState = 1;
        Touchkey_value_new = UNI_PCNT->VAL;   ///获取计数值
...
int plcnt_init(void *_data)
{
    ...
    Touchkey_value_old = UNI_PCNT->VAL;   ///获取计数值
    ...
    plcnt_ctm_debug("touch_key_init , plcnt_con = 0x%x, val = 0x%x", UNI_PCNT->CON, UNI_PCNT->VAL);
