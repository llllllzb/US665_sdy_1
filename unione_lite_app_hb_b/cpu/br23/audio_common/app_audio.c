...
s16 dac_buff[1 * 1024 * BNUM] SEC(.dac_buff);
#endif

static s8 *volumes4gear = NULL;
static u8 volume_gear = 0xFF;

static void _set_volumes(s8 volume)
{
  ...
  app_audio_set_volume(APP_AUDIO_STATE_WTONE, app_var.wtone_volume, 1);
  app_audio_set_volume(APP_AUDIO_STATE_CALL, app_var.call_volume, 1);
}

void user_set_volume(s8 volume)
{
  s8 vol = volume;
  if (volume > USER_AUDIO_VOL_MAX) {
    vol = USER_AUDIO_VOL_MAX;
  } else if (volume < USER_AUDIO_VOL_MIN) {
    vol = USER_AUDIO_VOL_MIN;
  }
  _set_volumes(vol);
  printf("user volume set: %d", vol);
}

static void _set_volume_gear(u8 gear)
{
  volume_gear = gear;
  _set_volumes(volumes4gear[volume_gear]);
  printf("volume gear set: %d(vol = %d)", gear, volumes4gear[volume_gear]);
}

static void calculate_volumes(void)
{
  float max_vol = USER_AUDIO_VOL_MAX;
  float min_vol = USER_AUDIO_VOL_MIN;
  float step = (max_vol - min_vol) / (USER_AUDIO_VOL_LEVEL - 1);
  int i;

  printf("max user volume: %d", (s8)max_vol);
  printf("min user volume: %d", (s8)min_vol);
  volumes4gear = (s8 *)malloc(sizeof(s8) * USER_AUDIO_VOL_LEVEL);
  if (volumes4gear == NULL) {
    printf("failed to malloc volumes4gear\n");
    return;
  }
  volumes4gear[0] = min_vol;
  volumes4gear[USER_AUDIO_VOL_LEVEL - 1] = max_vol;
  for (i = 1; i < USER_AUDIO_VOL_LEVEL - 1; i++) {
    volumes4gear[i] = (s8)(min_vol + step * i);
  }
  printf("%d-step volumes:", USER_AUDIO_VOL_LEVEL);
  for (i = 0; i < USER_AUDIO_VOL_LEVEL; i++) {
    printf("%d ", volumes4gear[i]);
  }
}

void set_volume_gear(u8 gear)
{
  if (gear > (USER_AUDIO_VOL_LEVEL - 1) || gear < 0)
    return;
  _set_volume_gear(gear);
}

u8 get_volume_gear(void)
{
  return volume_gear;
}

void volume_gear_up(void)
{
  int i;
  s8 vol;
  ...
  if (volume_gear == 0xFF) {
    ...
    for (i = 1; i < USER_AUDIO_VOL_LEVEL; i++) {
      if (volumes4gear[i] > vol) {
	volume_gear = i;
	_set_volume_gear(volume_gear);
	return;
      }
    }
  }
  ...
  volume_gear++;
  if (volume_gear > (USER_AUDIO_VOL_LEVEL - 1)) {
    volume_gear = (USER_AUDIO_VOL_LEVEL - 1);
  }
  _set_volume_gear(volume_gear);
}

void volume_gear_down(void)
{
  int i;
  s8 vol;
  ...
  if (volume_gear == 0xFF) {
    vol = app_audio_get_volume(APP_AUDIO_STATE_MUSIC);
    for (i = USER_AUDIO_VOL_LEVEL - 1; i > 0; i--) {
      if (volumes4gear[i] < vol) {
	volume_gear = i;
	_set_volume_gear(volume_gear);
	return;
...
{
    //DAC:DACEN
    UNI_AUDIO->DAC_CON &= ~BIT(4);
    //ADC:ADCEN
    UNI_AUDIO->ADC_CON &= ~BIT(4);
    //EQ:
    UNI_EQ->CON0 &= ~BIT(0);
    //FFT:
    UNI_FFT->CON = BIT(1);//置1强制关闭模块，不管是否已经运算完成
}
 ...
 *************************************************************
 */
static void set_pa(unsigned int val) {
    printf("wzc set gpio: %d\n\n", val);
#if 0
    gpio_direction_output(IO_PORTB_05, val);
    gpio_direction_output(IO_PORTB_01, val);
    gpio_direction_output(IO_PORTB_08, val);
    gpio_direction_output(IO_PORTB_09, val);
    gpio_direction_output(IO_PORTB_10, val);
    gpio_direction_output(IO_PORTB_11, val);
    gpio_direction_output(IO_PORTA_01, val);
    gpio_direction_output(IO_PORTA_00, val);
#else
    gpio_set_direction(IO_PORTB_05, 0);
    gpio_set_output_value(IO_PORTB_05, val);
    gpio_set_direction(IO_PORTB_01, 0);
    gpio_set_output_value(IO_PORTB_01, val);
    gpio_set_direction(IO_PORTB_08, 0);
    gpio_set_output_value(IO_PORTB_08, val);
    gpio_set_direction(IO_PORTB_09, 0);
    gpio_set_output_value(IO_PORTB_09, val);
    gpio_set_direction(IO_PORTB_10, 0);
    gpio_set_output_value(IO_PORTB_10, val);
    gpio_set_direction(IO_PORTB_11, 0);
    gpio_set_output_value(IO_PORTB_11, val);
    gpio_set_direction(IO_PORTA_01, 0);
    gpio_set_output_value(IO_PORTA_01, val);
    gpio_set_direction(IO_PORTA_00, 0);
    gpio_set_output_value(IO_PORTA_00, val);
    gpio_set_direction(IO_PORTC_07, 0);
    gpio_set_output_value(IO_PORTC_07, val);
#endif
}
...
{
    calculate_volumes();
    user_set_volume(USER_AUDIO_VOL_DEFAULT);
}
...
#define MIC_BIAS_RSEL(x) 	SFR(UNI_ANA->ADA_CON2, 21, 5, x)
#define MIC_LDO_SEL(x)		SFR(UNI_ANA->ADA_CON2, 16, 2, x)
