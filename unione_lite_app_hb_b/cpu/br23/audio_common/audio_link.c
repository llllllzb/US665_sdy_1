...
static void audio_link_clock_sel(u8 module, enum alnk_clock clk)
{
    ...
    case ALINK_CLOCK_12M288K:
        if (module) {
            SFR(UNI_CLOCK->CLK_CON2, 10, 4, 5); //160/13
        } else {
            SFR(UNI_CLOCK->CLK_CON2, 6, 4, 5); //160/13
        }
        break;
    case ALINK_CLOCK_11M2896K:
        if (module) {
            SFR(UNI_CLOCK->CLK_CON2, 10, 4, 0); //192/17
        } else {
            SFR(UNI_CLOCK->CLK_CON2, 6, 4, 0); //192/17
        }
        break;
    default:
        break;
...
{
    u8 module = ALINK_PORT_TO_MODULE(port);
    alink_printf("UNI_ALNK0->CON0 = 0x%x", ALINK_SEL(module, CON0));
    alink_printf("UNI_ALNK0->CON1 = 0x%x", ALINK_SEL(module, CON1));
    alink_printf("UNI_ALNK0->CON2 = 0x%x", ALINK_SEL(module, CON2));
    alink_printf("UNI_ALNK0->CON3 = 0x%x", ALINK_SEL(module, CON3));
    alink_printf("UNI_ALNK0->LEN  = 0x%x", ALINK_SEL(module, LEN));
    alink_printf("UNI_ALNK0->ADR0 = 0x%x", ALINK_SEL(module, ADR0));
    alink_printf("UNI_ALNK0->ADR1 = 0x%x", ALINK_SEL(module, ADR1));
    alink_printf("UNI_ALNK0->ADR2 = 0x%x", ALINK_SEL(module, ADR2));
    alink_printf("UNI_ALNK0->ADR3 = 0x%x", ALINK_SEL(module, ADR3));
    alink_printf("UNI_CLOCK->CLK_CON2 = 0x%x", UNI_CLOCK->CLK_CON2);
}
