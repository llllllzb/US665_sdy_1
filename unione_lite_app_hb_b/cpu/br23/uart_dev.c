...
{
    if (UNI_UART0->CON0 & BIT(0)) {
        UNI_UART0->BUF = a;
        __asm__ volatile("csync");
        while ((UNI_UART0->CON0 & BIT(15)) == 0);
        UNI_UART0->CON0 |= BIT(13);
    }
}
/**
 * @brief ut0接收一个byte
...
{
    u32 rx_len = 0;
    if ((UNI_UART0->CON0 & BIT(2)) && (UNI_UART0->CON0 & BIT(15))) {
        UNI_UART0->CON0 |= BIT(13);
    ...
    if ((UNI_UART0->CON0 & BIT(3)) && (UNI_UART0->CON0 & BIT(14))) {
        UNI_UART0->CON0 |= BIT(7);                     //DMA模式
        UNI_UART0->CON0 |= BIT(10);                    //清OTCNT PND
        UNI_UART0->CON0 |= BIT(12);                    //清RX PND(这里的顺序不能改变，这里要清一次)
        rx_len = UNI_UART0->HRXCNT;                    //读当前串口接收数据的个数
    ...
    if ((UNI_UART0->CON0 & BIT(5)) && (UNI_UART0->CON0 & BIT(11))) {
        //OTCNT PND
        UNI_UART0->CON0 |= BIT(7);                     //DMA模式
        UNI_UART0->CON0 |= BIT(10);                    //清OTCNT PND
        UNI_UART0->CON0 |= BIT(12);                    //清RX PND(这里的顺序不能改变，这里要清一次)
...
static void UT0_write_buf(const u8 *buf, u32 len)
{
    ...
    if (CONFIG_UART0_ENABLE_TX_DMA) {
        ...
        UNI_UART0->CON0 |= BIT(13);
        UNI_UART0->CON0 |= BIT(2);
        UNI_UART0->TXADR = (u32)buf;
        UNI_UART0->TXCNT = len;
...
{
    UNI_UART0->CON0 &= ~BIT(0);
    UNI_UART0->CON0 |= BIT(13) | BIT(12) | BIT(10);
    UNI_UART0->BAUD = ((UART_CLK + baud / 2) / baud) / 4 - 1;
    if (UNI_UART0->CON0 & BIT(5)) {
        if (uart0.rx_timeout > 10) {
            UNI_UART0->OTCNT = (uart0.rx_timeout / 10) * (UART_OT_CLK / 10) / 10;
        } else {
            UNI_UART0->OTCNT = uart0.rx_timeout * UART_OT_CLK / 1000;
        }
    }
    UNI_UART0->CON0 |= BIT(13) | BIT(12) | BIT(10) | BIT(0);
}
/**
 * @brief ut0使能
 */
static void UT0_open(u32 baud, u32 is_9bit, void *cbuf, u32 cbuf_size, u32 rx_cnt, u32 ot)
{
    ...
    if (cbuf) {
        ...
        uart0.rx_timeout = ot;
        UNI_UART0->RXSADR = (u32)uart0.kfifo.buffer;
        UNI_UART0->RXEADR = (u32)(uart0.kfifo.buffer + uart0.kfifo.buf_size);
        UNI_UART0->RXCNT = uart0.frame_length;
        UNI_UART0->CON0 |= BIT(6) | BIT(5) | BIT(3);
    }
    if (is_9bit) {
        UNI_UART0->CON0 |= BIT(1);
    } else {
        UNI_UART0->CON0 &= ~BIT(1);
    }
    UT0_set_baud(baud);
...
{
    if (UNI_UART1->CON0 & BIT(0)) {
        UNI_UART1->BUF = a;
        __asm__ volatile("csync");
        while ((UNI_UART1->CON0 & BIT(15)) == 0) {
            UNI_UART1->CON0 |= BIT(13);
            while (UNI_UART1->CON1 & BIT(2) && UNI_UART1->CON1 & BIT(15))	 {
                UNI_UART1->CON1 |= BIT(14);
                os_time_dly(1);
            }
        }
    }
...
{
    u32 rx_len = 0;
    if ((UNI_UART1->CON0 & BIT(2)) && (UNI_UART1->CON0 & BIT(15))) {
        UNI_UART1->CON0 |= BIT(13);
    ...
    if ((UNI_UART1->CON0 & BIT(3)) && (UNI_UART1->CON0 & BIT(14))) {
        UNI_UART1->CON0 |= BIT(7);                     //DMA模式
        UNI_UART1->CON0 |= BIT(10);                    //清OTCNT PND
        UNI_UART1->CON0 |= BIT(12);                    //清RX PND(这里的顺序不能改变，这里要清一次)
        rx_len = UNI_UART1->HRXCNT;                    //读当前串口接收数据的个数
        uart1.kfifo.buf_in += uart1.frame_length; //每满32字节则产生一次中断
        if (UNI_UART1->CON1 & BIT(0)) {
            UNI_UART1->CON1 |= BIT(13);
        }
    ...
    if ((UNI_UART1->CON0 & BIT(5)) && (UNI_UART1->CON0 & BIT(11))) {
        //OTCNT PND
        UNI_UART1->CON0 |= BIT(7);                     //DMA模式
        UNI_UART1->CON0 |= BIT(10);                    //清OTCNT PND
        UNI_UART1->CON0 |= BIT(12);                    //清RX PND(这里的顺序不能改变，这里要清一次)
...
static void UT1_write_buf(const u8 *buf, u32 len)
{
    ...
    if (CONFIG_UART1_ENABLE_TX_DMA) {
        ...
        UNI_UART1->CON0 |= BIT(13);
        UNI_UART1->CON0 |= BIT(2);
        UNI_UART1->TXADR = (u32)buf;
        UNI_UART1->TXCNT = len;
...
{
    UNI_UART1->CON0 &= ~BIT(0);
    UNI_UART1->CON0 |= BIT(13) | BIT(12) | BIT(10);
    UNI_UART1->BAUD = ((UART_CLK + baud / 2) / baud) / 4 - 1;
    if (UNI_UART1->CON0 & BIT(5)) {
        if (uart1.rx_timeout > 10) {
            UNI_UART1->OTCNT = (uart1.rx_timeout / 10) * (UART_OT_CLK / 10) / 10;
        } else {
            UNI_UART1->OTCNT = uart1.rx_timeout * UART_OT_CLK / 1000;
        }
    }
    UNI_UART1->CON0 |= BIT(13) | BIT(12) | BIT(10) | BIT(0);
}
/**
 * @brief ut1使能
 */
static void UT1_open(u32 baud, u32 is_9bit, void *cbuf, u32 cbuf_size, u32 rx_cnt, u32 ot)
{
    ...
    if (cbuf) {
        ...
        uart1.rx_timeout = ot;
        UNI_UART1->RXSADR = (u32)uart1.kfifo.buffer;
        UNI_UART1->RXEADR = (u32)(uart1.kfifo.buffer + uart1.kfifo.buf_size);
        UNI_UART1->RXCNT = uart1.frame_length;
        UNI_UART1->CON0 |= BIT(6) | BIT(5) | BIT(3);
    }
    if (is_9bit) {
        UNI_UART1->CON0 |= BIT(1);
    } else {
        UNI_UART1->CON0 &= ~BIT(1);
    }
    UT1_set_baud(baud);
...
{
    if (UNI_UART2->CON0 & BIT(0)) {
        UNI_UART2->BUF = a;
        __asm__ volatile("csync");
        while ((UNI_UART2->CON0 & BIT(15)) == 0);
        UNI_UART2->CON0 |= BIT(13);
    }
}
/**
 * @brief ut2接收一个byte
...
{
    u32 rx_len = 0;
    if ((UNI_UART2->CON0 & BIT(2)) && (UNI_UART2->CON0 & BIT(15))) {
        UNI_UART2->CON0 |= BIT(13);
    ...
    if ((UNI_UART2->CON0 & BIT(3)) && (UNI_UART2->CON0 & BIT(14))) {
        UNI_UART2->CON0 |= BIT(7);                     //DMA模式
        UNI_UART2->CON0 |= BIT(10);                    //清OTCNT PND
        UNI_UART2->CON0 |= BIT(12);                    //清RX PND(这里的顺序不能改变，这里要清一次)
        rx_len = UNI_UART2->HRXCNT;             //读当前串口接收数据的个数
    ...
    if ((UNI_UART2->CON0 & BIT(5)) && (UNI_UART2->CON0 & BIT(11))) {
        //OTCNT PND
        UNI_UART2->CON0 |= BIT(7);                     //DMA模式
        UNI_UART2->CON0 |= BIT(10);                    //清OTCNT PND
        UNI_UART2->CON0 |= BIT(12);                    //清RX PND(这里的顺序不能改变，这里要清一次)
...
static void UT2_write_buf(const u8 *buf, u32 len)
{
    ...
    if (CONFIG_UART2_ENABLE_TX_DMA) {
        ...
        UNI_UART2->CON0 |= BIT(13);
        UNI_UART2->CON0 |= BIT(2);
        UNI_UART2->TXADR = (u32)buf;
        UNI_UART2->TXCNT = len;
...
{
    UNI_UART2->CON0 &= ~BIT(0);
    UNI_UART2->CON0 |= BIT(13) | BIT(12) | BIT(10);
    UNI_UART2->BAUD = ((UART_CLK + baud / 2) / baud) / 4 - 1;
    if (UNI_UART2->CON0 & BIT(5)) {
        if (uart2.rx_timeout > 10) {
            UNI_UART2->OTCNT = (uart2.rx_timeout / 10) * (UART_OT_CLK / 10) / 10;
        } else {
            UNI_UART2->OTCNT = uart2.rx_timeout * UART_OT_CLK / 1000;
        }
    }
    UNI_UART2->CON0 |= BIT(13) | BIT(12) | BIT(10) | BIT(0);
}
/**
 * @brief ut2使能
 */
static void UT2_open(u32 baud, u32 is_9bit, void *cbuf, u32 cbuf_size, u32 rx_cnt, u32 ot)
{
    ...
    if (cbuf) {
        ...
        uart2.rx_timeout = ot;
        UNI_UART2->RXSADR = (u32)uart2.kfifo.buffer;
        UNI_UART2->RXEADR = (u32)(uart2.kfifo.buffer + uart2.kfifo.buf_size);
        UNI_UART2->RXCNT = uart2.frame_length;
        UNI_UART2->CON0 |= BIT(6) | BIT(5) | BIT(3);
    }
    if (is_9bit) {
        UNI_UART2->CON0 |= BIT(1);
    } else {
        UNI_UART2->CON0 &= ~BIT(1);
    }
    UT2_set_baud(baud);
...
static u32 uart_is_idle(u32 ut_num)
{
    switch (ut_num) {
    case 0 :
        return !(UNI_UART0->CON0 & BIT(0));
    case 1 :
        return !(UNI_UART1->CON0 & BIT(0));
    case 2:
        return !(UNI_UART2->CON0 & BIT(0));
    default :
        break;
    }
    return -1;
...
{
    if (UNI_UART1->CON1 & BIT(2)) {
        if (state) {
            UNI_UART1->CON1 |= BIT(4);
        } else {
            UNI_UART1->CON1 &= ~BIT(4);
        }
    }
