...
/**
 * 注意：UNI_WAKEUP 区别于PMU管理的唤醒。可理解为一个独立的模块使用。但在低功耗的情况下，中断无效。
 */
...
{
    if (UNI_WAKEUP->CON3 & BIT(0)) {
        UNI_WAKEUP->CON2 |= BIT(0);
...
{
    UNI_WAKEUP->CON0 &= ~BIT(0);
    gpio_set_die(port, 1);
    gpio_set_direction(port, 1);
    if (trigger_edge == 0) {
        UNI_WAKEUP->CON1 &= ~BIT(0);
    ...
    UNI_IOMAP->CON2 &= ~(0b111111 << 0);                 //使用inputchannel 0
    UNI_IOMAP->CON2 |= (port << 0);
    UNI_WAKEUP->CON2 |= BIT(0);                          //清一次pnd
    UNI_WAKEUP->CON0 |= BIT(0);                          //引脚中断使能
}
