...
int mic_sw_src_init(u16 out_sr)
{
    ...
    rs_para_obj.new_insample = uni_sr_table[get_sample_rate_index(MIC_EFFECT_SAMPLERATE)];
    rs_para_obj.new_outsample = uni_sr_table[get_sample_rate_index(out_sr)];
...
int audio_mic_open(struct adc_mic_ch *mic, u16 sample_rate, u8 gain)
{
    ...
    if (ladc_var.states == 0) {
        ...
        if (ladc_var.ladc_ch_num == 1) {
            ...
            UNI_ANA->ADA_CON1 |= BIT(17); //默认先关闭linein通路
            UNI_ANA->ADA_CON2 |= BIT(13);
