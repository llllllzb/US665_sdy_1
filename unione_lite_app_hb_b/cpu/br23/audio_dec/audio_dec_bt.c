...
static void a2dp_audio_res_close(void)
{
    if (bt_a2dp_dec->dec.start == 0) {
        TEST_PRINTF("bt_a2dp_dec->dec.start == 0");
        return ;
    }
...
{
        TEST_PRINTF("a2dp dec event handler:%d\n", argv[0]);
    switch (argv[0]) {
    case AUDIO_DEC_EVENT_END:
        TEST_PRINTF("AUDIO_DEC_EVENT_END\n");
        a2dp_dec_close();
        break;
    }
}
...
static int a2dp_dec_start(void)
{
    ...
    TEST_PRINTF("dec->ch:%d, fmt->channel:%d\n", dec->dec.ch, fmt->channel);
...
static int a2dp_wait_res_handler(struct audio_res_wait *wait, int event)
{
    ...
    TEST_PRINTF("a2dp_wait_res_handler: %d\n", event);
...
int a2dp_dec_open(int media_type)
{
    ...
    if (bt_a2dp_dec && (bt_a2dp_dec->dec.start == 0)) {
      TEST_PRINTF("a2dp drop frame start\n");
        a2dp_drop_frame_start();
    }
...
int a2dp_dec_close(void)
{
    ...
    clock_set_cur();
    TEST_PRINTF("a2dp_dec_close: exit\n");
    return 1;
}
...
static void *esco_plc_init(void)
{
    ...
    TEST_PRINTF("PLC_buf:0x%x,size:%d\n", (int)plc, PLC_query());
    if (!plc) {
        return NULL;
    }
    int err = PLC_init(plc);
    if (err) {
        TEST_PRINTF("PLC_init err:%d", err);
        free(plc);
        plc = NULL;
    }
    return plc;
...
static int esco_dec_start()
{
    ...
    if (err) {
        TEST_PRINTF("audio_aec_init failed:%d", err);
        //goto __err3;
    }
    err = esco_enc_open(dec->dec.coding_type, dec->dec.esco_len);
    if (err) {
        TEST_PRINTF("audio_enc_open failed:%d", err);
        //goto __err3;
    }
    dec->dec.enc_start = 1;
...
{
    int err = 0;
    TEST_PRINTF("esco_wait_res_handler %d\n", event);
    if (event == AUDIO_RES_GET) {
        err = esco_dec_start();
    } else if (event == AUDIO_RES_PUT) {
        if (bt_esco_dec->dec.start) {
...
int esco_dec_open(void *param, u8 mute)
{
    ...
    TEST_PRINTF("esco_dec_open, type=%d,len=%d\n", codec_type, esco_len);
...
void esco_dec_close()
{
    ...
    TEST_PRINTF("esco_dec_close: exit\n");
#if AUDIO_OUTPUT_AUTOMUTE
    extern void mix_out_automute_skip(u8 skip);
...
void audio_bt_effect_switch(u8 eff_mode)
{
    ...
    TEST_PRINTF("SPK eff***************:%d ", eff_mode);
#if AUDIO_SURROUND_CONFIG
