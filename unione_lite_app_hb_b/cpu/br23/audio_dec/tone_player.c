...
u8 tone_get_index(u16 name_digital)
{
	u8 index = IDEX_TONE_NONE;
	for (int i = 0; i < sizeof(tone_mapping)/sizeof(tone_mapping[0]); i++) {
		if (name_digital == tone_mapping[i].name) {
			index = tone_mapping[i].idx;
		}
	}
   return index;
}
