...
#define IRFLT_OUTPUT_TIMER_SEL(x)		SFR(UNI_IOMAP->CON0, 8, 3, x)
...
void set_ir_clk(void)
{
    ...
    UNI_IOMAP->CON1 |= BIT(27);//这里已选了timer3,时钟源选io信号里的pll_12m,不是所有的timer都可选pll,修改请看文档
...
{
    UNI_IR->RFLT_CON = 0;
    /* UNI_IR->RFLT_CON |= BIT(7);		//256 div */
    /* UNI_IR->RFLT_CON |= BIT(3);		//osc 24m */
    UNI_IR->RFLT_CON |= BIT(7) | BIT(4);		//512 div
    UNI_IR->RFLT_CON |= BIT(3) | BIT(2);		//PLL_48m（兼容省晶振）
    UNI_IR->RFLT_CON |= BIT(0);		//irflt enable
...
void log_irflt_info()
{
    ir_log("IOMC0 = 0x%x", UNI_IOMAP->CON0);
    ir_log("IOMC2 = 0x%x", UNI_IOMAP->CON2);
    ir_log("RFLT_CON = 0x%x", UNI_IR->RFLT_CON);
