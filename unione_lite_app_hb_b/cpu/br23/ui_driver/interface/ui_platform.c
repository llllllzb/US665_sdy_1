    ...
    if (!_head) {
        ...
        if (last_page == -1) { //第一次上电未初始化
            if (page == 0) { //表盘界面
                ...
                style.file = RES_PATH"UNI/UNI.sty";
            }
            load_style = 1;
...
static int br23_load_style(struct ui_style *style)
{
    ...
    if (style->file == NULL) {
        ...
        fscan_release(fs);
#else
        ui_file1 = res_fopen(RES_PATH"UNI.sty", "r");
        if (!ui_file1) {
            return -ENOENT;
        }
        ui_file_len = res_flen(ui_file1);
        len = 6;
        strcpy(style_name, "UNI.sty");
        if (len) {
            style_name[len - 4] = 0;
            ui_core_set_style(style_name);
        }
...
static int open_resource_file()
{
    ...
    ret = open_resfile(RES_PATH"UNI.res");
    if (ret) {
        return -EINVAL;
    }
    ret = open_str_file(RES_PATH"UNI.str");
    if (ret) {
        return -EINVAL;
    }
...
{
#if(CONFIG_UI_STYLE == STYLE_UNI_WTACH)
