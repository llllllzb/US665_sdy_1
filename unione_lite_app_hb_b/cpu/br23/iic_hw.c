...
static UNI_IIC_TypeDef *const iic_regs[IIC_HW_NUM] = {
    UNI_IIC,
};
...
static int iic_port_init(hw_iic_dev iic)
{
    ...
    if (id == 0) {
        ...
        if (port == 0) {
            ...
            UNI_USB_IO->CON0 |= (BIT(11) | BIT(8)); //IO MODE
            /* UNI_USB_IO->CON0 &= ~BIT(2);  //DP DIR OUT */
            /* UNI_USB_IO->CON0 &= ~BIT(3);  //DM DIR OUT */
            /* UNI_USB_IO->CON0 |= BIT(0);  //DP output 1 */
            /* UNI_USB_IO->CON0 |= BIT(1);  //DM output 1 */
            if (iic_info_io_pu(iic)) {
                UNI_USB_IO->CON0 |= BIT(6);  //DP PU
                UNI_USB_IO->CON0 |= BIT(7);  //DM PU
            } else {
                UNI_USB_IO->CON0 &= ~BIT(6);  //DP PU
                UNI_USB_IO->CON0 &= ~BIT(7);  //DM PU
            }
...
static void hw_iic_set_die(hw_iic_dev iic, u8 en)
{
    ...
    if (id == 0) {
        ...
        if (port == 0) {
            if (en) {
                UNI_USB_IO->CON0 |= BIT(9);  //DP 1.2V digital input en
                UNI_USB_IO->CON0 |= BIT(10);  //DM 1.2V digital input en
            } else {
                UNI_USB_IO->CON0 &= ~BIT(9);
                UNI_USB_IO->CON0 &= ~BIT(10);
            }
        } else {
...
#if 0
        if (port == 0) {
            //UNI_USB_IO->CON0 |= BIT(2);  //DP DIR IN
            //UNI_USB_IO->CON0 |= BIT(3);  //DM DIR IN
            UNI_USB_IO->CON0 &= ~(BIT(11) | BIT(8)); //disable IO_MODE
            UNI_USB_IO->CON0 &= ~BIT(6);  //DP PU
            UNI_USB_IO->CON0 &= ~BIT(7);  //DM PU
        } else {
