...
static void uart2_isr(void)
{
    ...
    if ((UNI_UART2->CON0 & BIT(2)) && (UNI_UART2->CON0 & BIT(15))) {//发送完成
        UNI_UART2->CON0 |= BIT(13);
        chargebox_data_deal(CMD_COMPLETE, EAR_L, NULL, 0);
    }
    if ((UNI_UART2->CON0 & BIT(3)) && (UNI_UART2->CON0 & BIT(14))) {//接收中断
        UNI_UART2->CON0 |= BIT(12);
        UNI_UART2->RXSADR = (u32)uart2_buf;
        UNI_UART2->RXEADR = (u32)(uart2_buf + BUF_LEN);
        UNI_UART2->RXCNT = BUF_LEN;
    ...
    if ((UNI_UART2->CON0 & BIT(5)) && (UNI_UART2->CON0 & BIT(11))) {
        ...
        UNI_UART2->OTCNT = timeout * (clk_get("uart") / 1000000);
...
static void uart1_isr(void)
{
    ...
    if ((UNI_UART1->CON0 & BIT(2)) && (UNI_UART1->CON0 & BIT(15))) {//发送完成
        UNI_UART1->CON0 |= BIT(13);
        /* printf("uart1 sned isr\n"); */
        chargebox_data_deal(CMD_COMPLETE, EAR_R, NULL, 0);
    }
    if ((UNI_UART1->CON0 & BIT(3)) && (UNI_UART1->CON0 & BIT(14))) {//接收中断
        /* putchar('Z'); */
        UNI_UART1->CON0 |= BIT(12);
        UNI_UART1->RXSADR = (u32)uart1_buf;
        UNI_UART1->RXEADR = (u32)(uart1_buf + BUF_LEN);
        UNI_UART1->RXCNT = BUF_LEN;
    ...
    if ((UNI_UART1->CON0 & BIT(5)) && (UNI_UART1->CON0 & BIT(11))) {
        ...
        UNI_UART1->OTCNT = timeout * (clk_get("lsb") / 1000000);
...
u8 chargebox_write(u8 l_r, u8 *data, u8 len)
{
    ...
    if (l_r == EAR_L) {
        ...
        UNI_UART2->TXADR = (u32)uart2_buf;
        UNI_UART2->TXCNT = (u16)len;
    } else {
        memcpy(uart1_buf, data, len);
        UNI_UART1->TXADR = (u32)uart1_buf;
        UNI_UART1->TXCNT = (u16)len;
    }
    return len;
}
...
void chargebox_set_baud(u8 l_r, u32 baudrate)
{
    ...
    if (l_r == EAR_L) {
        ...
        UNI_UART2->OTCNT = uart_timeout * (clk_get("lsb") / 1000000);
        UNI_UART2->BAUD = (clk_get("uart") / __this->uart2_baud) / 4 - 1;
        UNI_UART2->CON1 = (((clk_get("uart") / __this->uart2_baud) % 4) << 4);
    } else {
        __this->uart1_baud = baudrate;
        uart_timeout = 20 * 1000000 / __this->uart1_baud;
        UNI_UART1->OTCNT = uart_timeout * (clk_get("lsb") / 1000000);
        UNI_UART1->BAUD = (clk_get("uart") / __this->uart1_baud) / 4 - 1;
        UNI_UART1->CON1 = (((clk_get("uart") / __this->uart1_baud) % 4) << 4);
    }
}
...
void chargebox_init(const struct chargebox_platform_data *data)
{
    ...
    ASSERT(data);
    ASSERT(!(UNI_UART2->CON0 & BIT(0)), "uart2 already used!\n");
    ASSERT(!(UNI_UART1->CON0 & BIT(0)), "uart1 already used!\n");

    /////////////////////////////////////////////////////////////////////////////////////////////
    /* UNI_CLOCK->CLK_CON1 |= BIT(11); */
    /* UNI_CLOCK->CLK_CON1 &= ~BIT(10); */
    /////////////////////////////////////////////////////////////////////////////////////////////
    ...
    UNI_UART2->CON0 = BIT(13) | BIT(12) | BIT(10);
    UNI_UART2->OTCNT = uart_timeout * (clk_get("lsb") / 1000000);
    UNI_UART2->BAUD = (clk_get("uart") / __this->data->baudrate) / 4 - 1;
    UNI_UART2->CON1 = (((clk_get("uart") / __this->data->baudrate) % 4) << 4);
    gpio_set_pull_down(__this->data->L_port, 0);
    gpio_set_pull_up(__this->data->L_port, 1);
    gpio_set_die(__this->data->L_port, 1);
    ...
    UNI_IOMAP->CON3 &= ~BIT(11);//不占用IO

    //init uart1
    UNI_UART1->CON0 = BIT(13) | BIT(12) | BIT(10);
    UNI_UART1->OTCNT = uart_timeout * (clk_get("lsb") / 1000000);
    UNI_UART1->BAUD = (clk_get("uart") / __this->data->baudrate) / 4 - 1;
    UNI_UART1->CON1 = (((clk_get("uart") / __this->data->baudrate) % 4) << 4);
    gpio_set_pull_down(__this->data->R_port, 0);
    gpio_set_pull_up(__this->data->R_port, 1);
    gpio_set_die(__this->data->R_port, 1);
