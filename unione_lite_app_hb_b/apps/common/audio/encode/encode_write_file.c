...
int last_enc_file_path_get(char path[64])
{
...
#if TCFG_NOR_REC
    strcpy(logo, "rec_nor");
    sprintf(path, "%s%s%s", dev_manager_get_root_path_by_logo(logo), REC_FOLDER_NAME"/US66****",".WAV");
    ...
    sprintf(path, "%s%s%s%s", dev_manager_get_root_path_by_logo(logo), REC_FOLDER_NAME"/US66", index_str, ".WAV");
#endif
    printf("rec file =%s\n", path);
    return 0;
    ...
    if (enc_file_index != (u32) - 1) {
        ...
        if ((enc_file_coding_type == AUDIO_CODING_WAV) || (enc_file_coding_type == AUDIO_CODING_PCM)) {
            ...
            sprintf(path, "%s%s%s%s", dev_manager_get_root_path_by_logo(logo), REC_FOLDER_NAME"/US66", index_str, ".MP3");
        } else {
            return -1;
        }
        printf("rec file =%s\n", path);
...
void enc_w_test(void)
{
    ...
    printf("%s,%d ", __func__, __LINE__);
    tst->hdl = enc_write_file_open("sd0", "/"REC_FOLDER_NAME, "US660000.MP3");
    printf("%s,%d ", __func__, __LINE__);
    if (!tst->hdl) {
        goto __exit;
    }
