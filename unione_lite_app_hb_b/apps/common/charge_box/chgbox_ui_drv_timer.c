...
{
#if TIMER_USE_MC_PWM
    UNI_MCPWM->TMR0_CON |= BIT(10); //清pending
#else
    UNI_TIMERx->CON |= BIT(14);
#endif
    u8 i, led_idle = 0;
...
void mc_clk_init(void)
{
    ...
    UNI_MCPWM->TMR0_CON = BIT(10); //清pending,清其他bit
    UNI_MCPWM->MCPWM_CON0 = 0;
    ...
    UNI_MCPWM->TMR0_CNT = 0;
    UNI_MCPWM->TMR0_PR = prd_cnt;
    UNI_MCPWM->TMR0_CON |= index << 3; //分频系数

    request_irq(IRQ_MCTMRX_IDX, 3, soft_pwm_led_isr, 0);

    UNI_MCPWM->TMR0_CON |= BIT(8);  //允许定时溢出中断
    UNI_MCPWM->TMR0_CON |= BIT(0);  //递增模式

    UNI_MCPWM->MCPWM_CON0 |= BIT(8); //只开mc timer 0
    log_info("prd_cnt:%d,index:%d,t0:%x,MCP:%x\n", prd_cnt, index, UNI_MCPWM->TMR0_CON, UNI_MCPWM->MCPWM_CON0);
    log_info("lsb:%d\n", clk_get("lsb"));
#else
    u32 timer_clk;
    u32 prd_cnt;
    ...
    UNI_TIMERx->PRD = prd_cnt;
#if (TCFG_CLOCK_SYS_SRC == SYS_CLOCK_INPUT_PLL_RCL)
    UNI_IOMAP->CON0 |= BIT(TIMERX_IOS_BIT);
    UNI_TIMERx->CON = (index << 4) | BIT(0) | BIT(2);//省晶振使用pll12m为时钟源
#else
    UNI_TIMERx->CON = (index << 4) | (0b10 << 2) | (0b01 << 0);//OSC时钟
#endif
#endif
}
