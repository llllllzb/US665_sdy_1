...
static void chgbox_timer_delay_ms(u8 ms)
{
    ...
    UNI_TIMER2->CNT = 0;
#if (TCFG_CLOCK_SYS_SRC == SYS_CLOCK_INPUT_PLL_RCL)
    UNI_TIMER2->PRD = ms * clk_get("lsb") / (2 * 1000);
    UNI_TIMER2->CON = BIT(0) | BIT(6) | BIT(14); //系统时钟,lsb, div 2
#else
    UNI_TIMER2->PRD = ms * clk_get("timer") / 1000;
    UNI_TIMER2->CON = BIT(0) | BIT(3) | BIT(14); //1分频,osc时钟,24m，24次就1us
#endif
    while (!(UNI_TIMER2->CON & BIT(15))); //等pending
    UNI_TIMER2->CON = 0;
}
...
void chgbox_timer_delay_us(u8 us)
{
    ...
    UNI_TIMER2->CNT = 0;
#if (TCFG_CLOCK_SYS_SRC == SYS_CLOCK_INPUT_PLL_RCL)
    UNI_TIMER2->PRD = delay_tap[hs_cur_sys_clk][us];//这里不允许做乘除法
    UNI_TIMER2->CON = BIT(0) | BIT(6) | BIT(14); //系统时钟,lsb, div 2, 必须为24M
#else
    UNI_TIMER2->PRD = delay_tap[hs_cur_sys_clk][us];
    UNI_TIMER2->CON = BIT(0) | BIT(3) | BIT(14); //1分频,osc时钟,24m，24次就1us
#endif
    while (!(UNI_TIMER2->CON & BIT(15))); //等pending
    UNI_TIMER2->CON = 0;
