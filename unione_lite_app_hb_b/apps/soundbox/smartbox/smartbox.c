...
#ifndef UNI_SMART_BOX_CUSTOM_APP_EN
...
void UNI_rcsp_recieve_resume(void)
{
    int argv[3];
    argv[0] = (int)UNI_recieve_packet_parse_process;
    argv[1] = 1;
    argv[2] = 0;
    int ret = os_taskq_post_type("app_core", Q_CALLBACK, 3, argv);
}

void UNI_rcsp_resume_do(void)
{
    os_sem_post(&__this->sem);
    UNI_rcsp_recieve_resume();
}
...
static void smartbox_process(void *p)
{
    ...
    while (1) {
        ...
        UNI_send_packet_process();
        /* UNI_protocol_process(); */
    }
}
