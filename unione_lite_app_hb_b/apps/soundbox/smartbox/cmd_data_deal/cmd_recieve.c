...
static void get_target_feature(void *priv, u8 OpCode, u8 OpCode_SN, u8 *data, u16 len)
{
    ...
    UNI_CMD_response_send(OpCode, UNI_PRO_STATUS_SUCCESS, OpCode_SN, resp, wlen);
    free(resp);
}
...
static void get_sys_info(void *priv, u8 OpCode, u8 OpCode_SN, u8 *data, u16 len)
{
    ...
    UNI_CMD_response_send(OpCode, UNI_PRO_STATUS_SUCCESS, OpCode_SN, resp, (u16)rlen + 1);
    free(resp);
}
...
static void set_sys_info(void *priv, u8 OpCode, u8 OpCode_SN, u8 *data, u16 len)
{
    ...
    if (ret == true) {
        ...
        UNI_CMD_response_send(OpCode, UNI_PRO_STATUS_FAIL, OpCode_SN, NULL, 0);
    }
}
...
{
    /* printf("UNI_OPCODE_FILE_BROWSE_REQUEST_START\n"); */
    /* put_buf(data, len); */
#if RCSP_FILE_OPT
...
static void open_bt_scan(void *priv, u8 OpCode, u8 OpCode_SN, u8 *data, u16 len)
{
    ...
    UNI_CMD_response_send(OpCode, UNI_PRO_STATUS_SUCCESS, OpCode_SN, &result, 1);
}
...
static void get_md5_handle(void *priv, u8 OpCode, u8 OpCode_SN, u8 *data, u16 len)
{
    ...
    /* printf("get [md5] succ:"); */
    /* put_buf(md5, 32); */
    UNI_CMD_response_send(OpCode, UNI_PRO_STATUS_SUCCESS, OpCode_SN, md5, 32);
    fclose(fp);
}
...
static void get_low_latency_param(void *priv, u8 OpCode, u8 OpCode_SN, u8 *data, u16 len)
{
    ...
    UNI_CMD_response_send(OpCode, UNI_PRO_STATUS_SUCCESS, OpCode_SN, low_latency_param, 6);
}
