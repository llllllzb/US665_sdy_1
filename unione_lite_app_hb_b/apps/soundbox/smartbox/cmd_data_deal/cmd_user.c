...
/**@brief    smartbox自定义命令数据接收处理
   ...
   @return
   @note	 二次开发需要增加自定义命令，通过UNI_OPCODE_CUSTOMER_USER进行扩展，
...
*/
/*----------------------------------------------------------------------------*/
UNI_ERR smartbox_user_cmd_send(u8 *data, u16 len)
{
    //自定义数据接收
    printf("%s:", __FUNCTION__);
    put_buf(data, len);
    return UNI_CMD_send(UNI_OPCODE_CUSTOMER_USER, data, len, 1);
}
