...
void smartbox_command_send_bt_scan_result(char *name, u8 name_len, u8 *addr, u32 dev_class, char rssi)
{
    ...
    UNI_CMD_send(UNI_OPCODE_SYS_UPDATE_BT_STATUS, send_buf, send_len, UNI_NOT_NEED_RESPOND);
    free(send_buf);
    printf("bt name = %s\n", name);
}
...
void smartbox_command_send_conncecting_bt_status(u8 *addr, u8 status)
{
    ...
    UNI_CMD_send(UNI_OPCODE_SYS_EMITTER_BT_CONNECT_STATUS, send_buf, sizeof(send_buf), UNI_NOT_NEED_RESPOND);
}
...
static void smartbox_command_send_find_device(void *priv)
{
    ...
    UNI_CMD_send(UNI_OPCODE_SYS_FIND_DEVICE, send_buf, sizeof(send_buf), UNI_NOT_NEED_RESPOND);
}
