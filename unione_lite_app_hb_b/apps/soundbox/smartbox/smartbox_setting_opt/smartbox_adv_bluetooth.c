...
static u32 UNI_opcode_set_adv_info(void *priv, u8 OpCode, u8 OpCode_SN, u8 *data, u16 len)
{
    rcsp_printf("UNI_opcode_set_adv_info:\n");
    ...
    if (adv_setting_result) {
        ...
        UNI_CMD_response_send(OpCode, UNI_PRO_STATUS_SUCCESS, OpCode_SN, &ret, 1);
    } else {
        UNI_CMD_response_send(OpCode, UNI_PRO_STATUS_SUCCESS, OpCode_SN, &ret, 1);
    }
    return 0;
...
static u32 UNI_opcode_get_adv_info(void *priv, u8 OpCode, u8 OpCode_SN, u8 *data, u16 len)
{
    u8 buf[256];
    u8 offset = 0;
    ...
    ret = UNI_CMD_response_send(OpCode, UNI_PRO_STATUS_SUCCESS, OpCode_SN, buf, offset);
...
{
    return UNI_CMD_send(UNI_OPCODE_ADV_DEVICE_NOTIFY, buf, len, UNI_NOT_NEED_RESPOND);
}

u8 adv_info_device_request(u8 *buf, u16 len)
{
    printf("UNI_OPCODE_ADV_DEVICE_REQUEST\n");
    return UNI_CMD_send(UNI_OPCODE_ADV_DEVICE_REQUEST, buf, len, UNI_NEED_RESPOND);
}

int UNI_smartbox_adv_cmd_resp(void *priv, u8 OpCode, u8 OpCode_SN, u8 *data, u16 len)
{
    rcsp_printf("UNI_rcsp_adv_cmd_resp\n");
    switch (OpCode) {
    case UNI_OPCODE_SET_ADV:
        rcsp_printf(" UNI_OPCODE_SET_ADV\n");
        UNI_opcode_set_adv_info(priv, OpCode, OpCode_SN, data, len);
        break;
    case UNI_OPCODE_GET_ADV:
        rcsp_printf(" UNI_OPCODE_GET_ADV\n");
        UNI_opcode_get_adv_info(priv, OpCode, OpCode_SN, data, len);
        break;
    case UNI_OPCODE_ADV_NOTIFY_SETTING:
        rcsp_printf(" UNI_OPCODE_ADV_NOTIFY_SETTING\n");
        bt_ble_adv_ioctl(BT_ADV_SET_NOTIFY_EN, *((u8 *)data), 1);
        UNI_CMD_response_send(OpCode, UNI_PRO_STATUS_SUCCESS, OpCode_SN, NULL, 0);
        break;
    case UNI_OPCODE_ADV_DEVICE_REQUEST:
        rcsp_printf("UNI_OPCODE_ADV_DEVICE_REQUEST\n");
        break;
    case UNI_OPCODE_SET_DEVICE_REBOOT:
        rcsp_printf("UNI_OPCODE_SET_DEVICE_REBOOT\n");
        UNI_CMD_response_send(OpCode, UNI_PRO_STATUS_SUCCESS, OpCode_SN, NULL, 0);
        UNI_rcsp_event_to_user(DEVICE_EVENT_FROM_RCSP, MSG_UNI_REBOOT_DEV, NULL, 0);
        break;
    default:
        return 1;
    }
...
{
    switch (rcsp->event) {
    case MSG_UNI_REBOOT_DEV:
        smartbox_rcsp_reboot_dev();
        break;
    case MSG_UNI_UPDATE_SEQ:
        /* printf("MSG_UNI_UPDATE_SEQ\n"); */
        /* extern void adv_seq_vaule_sync(void); */
        /* adv_seq_vaule_sync(); */
        break;
    case MSG_UNI_UPDAET_ADV_STATE_INFO:
        /* if (get_rcsp_connect_status()) { */
        /* 	UNI_rcsp_set_auth_flag(rcsp->args[0]);	 */
...
void app_core_data_for_set(u8 *packet, u16 size)
{
    ...
    if (RCSP_SPP == bt_3th_get_cur_bt_channel_sel()) {
        UNI_rcsp_set_auth_flag(rcsp_auth_flag);
