...
{
    struct UNI_FILE_DATA file_data;
    memset((u8 *)&file_data, 0, sizeof(struct UNI_FILE_DATA));
    ...
    memcpy(dest + offset, (u8 *)&file_data, sizeof(struct UNI_FILE_DATA));
    memcpy(dest + offset + sizeof(struct UNI_FILE_DATA), (u8 *)p_dir_info->lfn_buf.lfn, p_dir_info->lfn_buf.lfn_cnt);

    /* printf("add send data:"); */
    /* put_buf(dest+offset,p_dir_info->lfn_buf.lfn_cnt + sizeof(struct UNI_FILE_DATA)); */
    return (p_dir_info->lfn_buf.lfn_cnt + sizeof(struct UNI_FILE_DATA));
}
...
static void smartbox_browser_task(void *p)
{
    ...
    for (int i = browser->start_num ; i < (browser->start_num + browser->read_file_num); i++) {
        ...
        if (offset && ((offset + dir_info.lfn_buf.lfn_cnt + sizeof(struct UNI_FILE_DATA)) > FILE_BROWSE_BUF_LEN)) {
            ///如果buf不够填充了， 先将数据发送了先，再重新填充
            ret = UNI_DATA_send(UNI_OPCODE_DATA, UNI_OPCODE_FILE_BROWSE_REQUEST_START, path_data, offset, UNI_NOT_NEED_RESPOND);
            if (ret) {
