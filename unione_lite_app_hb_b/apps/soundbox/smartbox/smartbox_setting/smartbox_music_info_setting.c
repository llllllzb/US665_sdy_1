...
void rcsp_update_player_state(void)
{
    ...
    UNI_rcsp_event_to_user(DEVICE_EVENT_FROM_RCSP, MSG_UNI_UPDATE_PLAYER_STATE, NULL, 0);
}
...
void music_info_cmd_handle(u8 *p, u16 len)
{
    ...
    case 0x04:
        ...
        if (*data) {
            UNI_rcsp_event_to_user(DEVICE_EVENT_FROM_RCSP, MSG_UNI_UPDATE_MUSIC_PLAYER_TIME_TEMER, data, 1);
        } else {
            UNI_rcsp_event_to_user(DEVICE_EVENT_FROM_RCSP, MSG_UNI_UPDATE_MUSIC_PLAYER_TIME_TEMER, data, 1);
        }
        break;
...
static void rcsp_adv_update_music_info(void *priv)
{
    ...
    if (music_info.player_time_en) {
        for (i = 0; i < (sizeof(type) * 8); i++) {
            if (type & BIT(i)) {
                UNI_rcsp_event_to_user(DEVICE_EVENT_FROM_RCSP, MSG_UNI_UPDATE_MUSIC_INFO, &i, 1);
            }
        }
    }
}
...
void rcsp_adv_music_info_deal(u8 type, u32 time, u8 *info, u16 len)
{
    ...
    case 0:
        if (time && time != music_info.total_time) {
            /* if(music_info.player_time_en) */
            {
                ...
                UNI_rcsp_event_to_user(DEVICE_EVENT_FROM_RCSP, MSG_UNI_UPDATE_PLAYER_TIME, NULL, 0);
            }
        }
        return;
    case 1:
    ...
    if (music_info.player_time_en) {
        UNI_rcsp_event_to_user(DEVICE_EVENT_FROM_RCSP, MSG_UNI_UPDATE_MUSIC_INFO, &type, 1);
    }
    /* os_time_dly(2); */
}
