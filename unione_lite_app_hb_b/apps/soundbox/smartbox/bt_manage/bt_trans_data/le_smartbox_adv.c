...
static void bt_ble_rcsp_adv_enable_do(void)
{
    ...
    extern u8 UNI_rcsp_get_auth_flag(void);
#if (!MUTIl_CHARGING_BOX_EN) && (RCSP_ADV_EN) && (TCFG_USER_TWS_ENABLE)
    if ((smartbox_get_con_handle() || get_rcsp_connect_status()) && UNI_rcsp_get_auth_flag()) {
        /* extern void deal_adv_setting_gain_time_stamp(void); */
        /* deal_adv_setting_gain_time_stamp(); */
    }
#endif
    if ((smartbox_get_con_handle() || get_rcsp_connect_status()) && UNI_rcsp_get_auth_flag() && (__this->ble_adv_notify || __this->modify_flag)) {
...
static void deal_sibling_seq_rand_sync(void *data, u16 len, bool rx)
{
    ...
    if (rx) {
        ...
        case TWS_ADV_SEQ_CHANGE:
            ...
            UNI_rcsp_event_to_user(DEVICE_EVENT_FROM_RCSP, MSG_UNI_UPDATE_SEQ, NULL, 0);
            break;
...
extern u8 uni_app_init_setting(void);
u8 btstck_app_init_setting(void)
{
    uni_app_init_setting();
    return 1;
}
...
{
    if (rx) {
        UNI_rcsp_event_to_user(DEVICE_EVENT_FROM_RCSP, MSG_UNI_UPDAET_ADV_STATE_INFO, data, len);
    }
}
...
static void adv_state_info_sync(void)
{
    ...
    if (get_bt_tws_connect_status()) {
        u8 param[2] = {0};
        param[0] = UNI_rcsp_get_auth_flag();
