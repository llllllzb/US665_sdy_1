...
u8 get_rcsp_connect_status(void)
{
    ...
    if (RCSP_BLE == bt_3th_get_cur_bt_channel_sel()) {
        if (bt_3th_get_uni_ble_status() == BLE_ST_CONNECT || bt_3th_get_uni_ble_status() == BLE_ST_NOTIFY_IDICATE) {
            return 1;
        } else {
            return 0;
        }
    } else {
        return bt_3th_get_uni_spp_status();
    }
#else
    return 0;
#endif
...
{
    UNI_rcsp_event_to_user(DEVICE_EVENT_FROM_RCSP, MSG_UNI_SWITCH_DEVICE, NULL, 0);
}
...
int UNI_rcsp_event_handler(struct rcsp_event *rcsp)
{
    int ret = 0;
    switch (rcsp->event) {
    case MSG_UNI_ADV_SETTING_SYNC:
        update_smartbox_setting(-1);
        break;
    case MSG_UNI_ADV_SETTING_UPDATE:
        update_info_from_vm_info();
        break;
    case MSG_UNI_SWITCH_DEVICE:
        printf("MSG_UNI_SWITCH_DEVICE\n");
    ...
    default:
        ...
        if (0 == UNI_smartbox_adv_event_handler(rcsp)) {
            break;
        }
#endif
#if RCSP_UPDATE_EN
        if (0 == UNI_rcsp_update_msg_deal(NULL, rcsp->event, rcsp->args)) {
            break;
        }
#endif
        break;
...
static void smartbox_user_state_event(u8 opcode, u8 state)
{
    ...
    data[0]	= opcode;
    data[1]	= state;
    UNI_rcsp_event_to_user(DEVICE_EVENT_FROM_RCSP, MSG_UNI_USER_SPP_BLE_STATE, data, sizeof(data));
}
