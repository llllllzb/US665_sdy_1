...
void file_delete_start(u8 OpCode_SN, u8 *data, u16 len)
{
    ...
    if (__this->fsn == NULL) {
        ...
        if (__this->busy) {
            ...
            UNI_CMD_response_send(UNI_OPCODE_FILE_DELETE, UNI_PRO_STATUS_BUSY, OpCode_SN, NULL, 0);
            return ;
        }
    }
    ...
    if (file) {
        ...
        UNI_CMD_response_send(UNI_OPCODE_FILE_DELETE, UNI_PRO_STATUS_SUCCESS, OpCode_SN, NULL, 0);
        if (last == 0) {
        ...
        UNI_CMD_response_send(UNI_OPCODE_FILE_DELETE, UNI_PRO_STATUS_PARAM_ERR, OpCode_SN, NULL, 0);
    }
    file_delete_end();
    return ;

__err:
    UNI_CMD_response_send(UNI_OPCODE_FILE_DELETE, UNI_PRO_STATUS_FAIL, OpCode_SN, NULL, 0);
    file_delete_end();
}
