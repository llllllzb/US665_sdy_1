...
void dev_format_start(u8 OpCode_SN, u8 *data, u16 len)
{
    ...
    if (logo == NULL) {
        ...
        UNI_CMD_response_send(UNI_OPCODE_DEVICE_FORMAT, UNI_PRO_STATUS_FAIL, OpCode_SN, &reason, 1);
        dev_format_close();
        return ;
    }
    ...
    if (root_path) {
        ...
        if (err) {
            ...
            UNI_CMD_response_send(UNI_OPCODE_DEVICE_FORMAT, UNI_PRO_STATUS_SUCCESS, OpCode_SN, &reason, 1);
            dev_format_close();
            return ;
        }
    } else {
