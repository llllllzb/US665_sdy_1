...
#define FTP_FILE_VAILD_MARK						"UNI_FTP"  //断点续传需要用到的文件内容标记
...
static void __file_transfer_download_file_check_caculate(void *priv)
{
    ...
    if (ftp_d->check.counter == 0) {
        ...
        if (ftp_d->check.crc_tmp == ftp_d->file_crc) {
            ...
            //进入重命名流程
            err = UNI_CMD_send(UNI_OPCODE_FILE_RENAME, NULL, 0, 1);
            if (err) {
                //命令发送失败， 直接停止
                file_transfer_close();
            }
            ...
            fdelete(ftp_d->file);
            ftp_d->file = NULL;
            UNI_ERR err = UNI_CMD_send(UNI_OPCODE_FILE_TRANSFER_END, &reason, 1, 1);
            if (err) {
                //命令发送失败， 直接停止
                file_transfer_close();
            }
...
static void file_transfer_download_get_data(void)
{
    ...
    UNI_ERR err = UNI_CMD_send(UNI_OPCODE_FILE_TRANSFER, parm, sizeof(parm), 0);
    if (err) {
        ftp_printf("%s fail!!! %d\n", __FUNCTION__, err);
        return;
    }
...
void file_transfer_download_start(u8 OpCode_SN, u8 *data, u16 len)
{
    ...
    if (ftp_d == NULL) {
        ...
        if (ftp_d) {
            printf("%s, no mem\n", __FUNCTION__);
            UNI_CMD_response_send(UNI_OPCODE_FILE_TRANSFER_START, UNI_PRO_STATUS_FAIL, OpCode_SN, NULL, 0);
            return ;
        }
    }
    ...
    err = UNI_CMD_response_send(UNI_OPCODE_FILE_TRANSFER_START, UNI_PRO_STATUS_SUCCESS, OpCode_SN, (u8 *)&file_data_unit, sizeof(u16));
    if (err == 0) {
...
{
    if (ftp_d)	{
        if (status == UNI_PRO_STATUS_SUCCESS)	{
            printf("%s !!, %d\n", __FUNCTION__, __LINE__);
            put_buf(data, len);
            //重命名
            if (file_rename((const char *)data) == 0) {
                u8 reason = FTP_END_REASON_NONE;
                int err = UNI_CMD_send(UNI_OPCODE_FILE_TRANSFER_END, &reason, 1, 1);
                if (err) {
                    file_transfer_close();
                }
                ...
                int err = UNI_CMD_send(UNI_OPCODE_FILE_RENAME, NULL, 0, 1);
                if (err) {
                    file_transfer_close();
                }
            }
...
static void file_transfer_download_doing_last_packet(u8 *data, u16 len)
{
    ...
    if (len > FTP_FILE_DATA_UNIT) {
        ...
        err = UNI_CMD_send(UNI_OPCODE_FILE_TRANSFER_END, &reason, 1, 1);
        if (err) {
            //命令发送失败， 直接停止
            file_transfer_close();
        }
...
void file_transfer_download_passive_cancel(u8 OpCode_SN, u8 *data, u16 len)
{
    ...
    UNI_CMD_response_send(UNI_OPCODE_FILE_TRANSFER_CANCEL, UNI_PRO_STATUS_SUCCESS, OpCode_SN, NULL, 0);
}
...
void file_transfer_download_active_cancel(void)
{
    if (ftp_d) {
        file_transfer_close();
        UNI_CMD_send(UNI_OPCODE_FILE_TRANSFER_CANCEL, NULL, 0, 1);
    }
}
