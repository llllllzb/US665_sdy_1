...
{
    UI_HIDE_CURR_WINDOW();
    tone_play_stop_by_path(tone_index[IDEX_TONE_MUSIC]);//停止播放提示音
    char *logo = music_player_get_dev_cur();
    if (music_player_get_playing_breakpoint(breakpoint, 1) == true) {
        breakpoint_vm_write(breakpoint, logo);
    }
...
#endif
    {
        tone_play_with_callback_by_name(tone_index[IDEX_TONE_MUSIC], 1, music_tone_play_end_callback, (void *)IDEX_TONE_MUSIC);
    }
#endif
