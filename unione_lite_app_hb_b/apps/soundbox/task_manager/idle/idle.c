...
static int idle_timer2_init()
{
    ...
    UNI_TIMER2->CNT = 0;
    UNI_TIMER2->PRD = prd_cnt; //2ms
    request_irq(IRQ_TIME2_IDX, 3, idle_timer2_isr, 0);
    UNI_TIMER2->CON = (index << 4) | BIT(0) | BIT(3);

    log_info("PRD : 0x%x / %d", UNI_TIMER2->PRD, clk_get("timer"));
...
{
    UNI_TIMER2->CON &= ~(BIT(1) | BIT(0));
}

static void idle_timer1_open()
{
    UNI_TIMER1->CON |= BIT(0);
}

static void idle_timer1_close()
{
    UNI_TIMER1->CON &= ~(BIT(1) | BIT(0));
}
...
void resume_some_peripheral()
{
    ...
    UNI_ANA->DAA_CON0 = *regs_ptr++ ;
    UNI_ANA->DAA_CON1 = *regs_ptr++ ;
    UNI_ANA->DAA_CON2 = *regs_ptr++ ;
    UNI_ANA->DAA_CON3 = *regs_ptr++ ;
    UNI_ANA->DAA_CON7 = *regs_ptr++ ;

    UNI_ANA->ADA_CON0 = *regs_ptr++ ;
    UNI_ANA->ADA_CON1 = *regs_ptr++ ;
    UNI_ANA->ADA_CON2 = *regs_ptr++ ;
    UNI_ANA->ADA_CON3 = *regs_ptr++ ;
    UNI_ANA->ADA_CON4 = *regs_ptr++ ;
...
void suspend_some_peripheral()
{
    ...
    *regs_ptr++ = UNI_ANA->DAA_CON0;
    *regs_ptr++ = UNI_ANA->DAA_CON1;
    *regs_ptr++ = UNI_ANA->DAA_CON2;
    *regs_ptr++ = UNI_ANA->DAA_CON3;
    *regs_ptr++ = UNI_ANA->DAA_CON7;

    *regs_ptr++ = UNI_ANA->ADA_CON0;
    *regs_ptr++ = UNI_ANA->ADA_CON1;
    *regs_ptr++ = UNI_ANA->ADA_CON2;
    *regs_ptr++ = UNI_ANA->ADA_CON3;
    *regs_ptr++ = UNI_ANA->ADA_CON4;
