...
//指定搜索uuid
//指定搜索uuid
static const target_uuid_t  uni_search_uuid_table[] = {
...
static void client_event_callback(le_client_event_e event, u8 *packet, int size)
{
    ...
    case CLI_EVENT_MATCH_UUID: {
        ...
        if (opt_hdl->search_uuid == &uni_search_uuid_table[0]) {
...
static const client_conn_cfg_t client_conn_config = {
    ...
    .search_uuid_cnt = (sizeof(uni_search_uuid_table) / sizeof(target_uuid_t)),
    .search_uuid_table = uni_search_uuid_table,
...
#endif
    char fix_mac[6] = {0x97, 0x67, 0xB2, 0x6A, 0x2E, 0xCF};      
    bt_update_mac_addr(fix_mac);
    extern u8 get_tws_background_connected_flag();
    if (!__this->cmd_flag && (!get_tws_background_connected_flag())) { //蓝牙后台拉回蓝牙模式不播放提示音
        tone_play_with_callback_by_name(tone_index[IDEX_TONE_BT_MODE], 1, bt_tone_play_end_callback, (void *)IDEX_TONE_BT_MODE);
        //协议栈初始化在提示音结束进行
    } else {
